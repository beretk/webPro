...
class App extends Component{
  constructor(props){
    super(props);
    this.maxId = 4; // state에 넣으면 불필요한 rendering 이루어짐을 막고자 밖에다
    this.state = {
      mode : 'welcome',
    ...
    if(this.state.mode === 'welcome'){
      ...
      _article = <CreateContent 
                  onSubmitCreate = {function(_title, _desc){
                    //alert(_title + ',' + _desc);
                    // _title과 _desc로 {id:5, title:_title, desc:_desc}를 this.state.contents에 추가
                    this.maxId ++;
                    ...
                    var _contents = this.state.contents.concat(_content); // this.state.contents변경안됨
                    this.setState({
                      contents : _contents,
                      mode : 'read',
      ...
      if(_content){
        _article = <UpdateContent data={_content}
                    onSubmitUpdate = {function(_id, _title, _desc){
                      _content = {id:Number(_id), title:_title, desc:_desc}; // 바꿀 내용, _id는 Number로형변환
                      var _contents = Array.from(this.state.contents);//성능 튜닝할지 모르니 복제
                      for(var idx=0; idx<_contents.length; idx++){
                        if(_contents[idx].id === Number(_id)){ //반드시 _id는 Number로형변환
