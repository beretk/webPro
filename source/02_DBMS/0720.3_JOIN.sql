...
SELECT ENAME, SAL, GRADE, D.DEPTNO, DNAME, LOC 
    FROM EMP E, DEPT D, SALGRADE --14* 4*15개=840
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER. ENAME  
    ...
    SELECT W.EMPNO, W.ENAME, M.ENAME
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO; --13행 (KING의 MGR이 NULL.EMPNO는 NULL 불가)
...
-- 탄탄3. SCOTT과 동일한 근무지에서 근무하는 사원의 이름을 출력하시오(탄탄2의 심화문제)
    ...
    -- 데이터 원상 복귀(DEPT 50번 INSERT, EMP 8000번 사우너 INSERT)
    ROLLBACK;
    SELECT * FROM DEPT;
    SELECT * FROM EMP;
    
-- ★ 4. OUTER JOIN : SELF JOIN 이나 EQUI JOIN 시 조건에 만족하지 않는 행까지 나타나게 함
...
--3. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력.
    --상사가 없을 시 직속상사명을 '없음'으로 출력) 
SELECT W.ENAME, W.SAL, W.JOB, NVL(M.ENAME,'없음') MESSAGE 
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO(+);
...
--9. 8번을 부서명 순 부서가 같으면 급여가 큰 순 SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM, 0))*12 ANNUALSAL, M.ENAME MANAGER
SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM, 0))*12 ANNUALSAL, M.ENAME MANAGER
    FROM EMP W, EMP M, SALGRADE, DEPT D
    WHERE W.MGR=M.EMPNO AND W.SAL BETWEEN LOSAL AND HISAL AND W.DEPTNO=D.DEPTNO
