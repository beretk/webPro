...
SELECT COUNT(*) FROM EMP; -- EMP 테이블의 행수
    ...
    SELECT TRUNC(SYSDATE-MIN(HIREDATE))||':일째', TRUNC(SYSDATE-MAX(HIREDATE))||':일째'FROM EMP;
    ...
    SELECT TO_CHAR(MIN(HIREDATE), 'RR"년"MM"월"DD" 최초입사:"') ||
...
-- ★ 2. GROUP BY절
    ...
    SELECT DEPTNO, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY DEPTNO;
        -- ※ GROUP BY절에서는 필드의 별칭을 사용없음 반드시 필드 이름 사용(ORDER BY절에서는 필드의 별칭 사용가능)
    SELECT DEPTNO 부서, MAX(SAL) MAX, AVG(SAL) AVG
        FROM EMP
        GROUP BY DEPTNO
        ORDER BY MAX(SAL); --Alias도 사용가능
...
SELECT DEPTNO, JOB, SUM(SAL) FROM EMP GROUP BY ROLLUP(DEPTNO, JOB); -- ROLLUP을 하면 ORDER BY도 자동으로 해줌
