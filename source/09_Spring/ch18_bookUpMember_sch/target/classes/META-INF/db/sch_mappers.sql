  ...
  SELECT * 
    FROM (SELECT ROWNUM RN, A.* 
          FROM (SELECT * FROM BOOK 
		        WHERE BTITLE LIKE '%'||'S'||'%' OR BINFO LIKE '%'||'S'||'%' ORDER BY BTITLE) A)
		  	WHERE RN BETWEEN 4 AND 6;
  -- schItem이 btitle일 때
  SELECT * 
    FROM (SELECT ROWNUM RN, A.* 
          FROM (SELECT * FROM BOOK 
		        WHERE BTITLE LIKE '%'||'S'||'%' ORDER BY BTITLE) A)
		  	WHERE RN BETWEEN 4 AND 6;
  -- schItem이 bwriter일 때
  SELECT * 
    FROM (SELECT ROWNUM RN, A.* 
          FROM (SELECT * FROM BOOK 
		        WHERE BWRITER LIKE '%'||'홍'||'%' ORDER BY BTITLE) A)
...
-- Book.xml id=totCntBook (등록된 책 갯수)
  ...
  -- schItem이 btitle일 때
  SELECT COUNT(*) FROM BOOK WHERE BTITLE LIKE '%'||'S'||'%';
  -- schItem이 bwriter일 때
