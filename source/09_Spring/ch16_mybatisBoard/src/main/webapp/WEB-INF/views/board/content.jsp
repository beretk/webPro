...
<body>
	...
	<table>
		...
		<tr>
			<th>첨부파일</th>
			<td>
				<c:if test="${empty contentBoard.bfileName }">
					첨부파일 없음
				</c:if>
				<c:if test="${not empty contentBoard.bfileName }">
					<a href="${conPath }/upload/${contentBoard.bfileName}">${contentBoard.bfileName }</a>
				</c:if>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="2">
				<button onclick="location.href='${conPath}/board/modify.do?bid=${param.bid}&pageNum=${param.pageNum}'">
					수정
				</button>
