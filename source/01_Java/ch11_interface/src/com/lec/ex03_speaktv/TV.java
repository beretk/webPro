...
public class TV implements IVolume {
	...
	public void volumeUp(int level) {
		if(volumeLevel + level < TV_MAX_VOLUME) {
			...
			System.out.println("TV볼륨을"+ tempLevel + "만큼 올려도 최대치("+ TV_MAX_VOLUME+")입니");
		}		
	}
	@Override
	public void volumeDown() {
		if(volumeLevel > TV_MIN_VOLUME) {
			volumeLevel--;
			System.out.println("TV볼륨을 1만큼내려 현재보륨"+ volumeLevel);
		}else {
	...
	public void volumeDown(int level) {
		if(volumeLevel -level > TV_MIN_VOLUME	) {
			...
			System.out.println("TV볼륨을"+ tempLevel + "만큼내려 현재 볼륨이 최소치("+ TV_MIN_VOLUME+")");
		}//if		
	}//volumeDown
}//class
