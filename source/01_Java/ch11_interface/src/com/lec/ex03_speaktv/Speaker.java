...
public class Speaker implements IVolume {
	...
	public void volumeUp() {
		if(volumeLevel < IVolume.SPEAKER_MAX_VOLUME) {
			volumeLevel++;
			System.out.println("SPEAKER 볼륨을 1만큼 올려현재 볼륨은"+volumeLevel);
		}else {
	...
	public void volumeUp(int level) {
		if(volumeLevel + level < SPEAKER_MAX_VOLUME) {
			...
			System.out.println("SPEAKER볼륨을"+ tempLevel + "만큼 올려도 최대치("+ SPEAKER_MAX_VOLUME+")입니");
		}		
	}
	@Override
	public void volumeDown() {
		if(volumeLevel > SPEAKER_MIN_VOLUME) {
			volumeLevel--;
			System.out.println("SPEAKER볼륨을 1만큼내려 현재보륨"+ volumeLevel);
		}else {
	...
	public void volumeDown(int level) {
		if(volumeLevel -level > SPEAKER_MIN_VOLUME	) {
			...
			System.out.println("SPEAKER볼륨을"+ tempLevel + "만큼내려 현재 볼륨이 최소치("+ SPEAKER_MIN_VOLUME+")");
		}//if		
	}//volumeDown
}//class
