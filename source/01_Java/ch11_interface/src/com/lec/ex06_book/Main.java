...
public class Main {
	public static void main(String[] args) {
		...
		do {
			...
			fn = scanner.nextInt();//white-space 앞까지 입력된 정수를 받음
			switch(fn) {
			case 1: //대출1. 책이름입력 2.책조회 3.책상태 확인 4.대출인 5.대출일 6.checkOut(대출인,대출일)메소드 호출
				//1.책이름 입력
				System.out.print("대출할 책 이름은?");
				bTitle = scanner.next();// "oracle" white-space 앞까지 입력된 스트링을 받음
				//2.책조회
				...
				break;
			case 2: //반납: 1. 책이름 2. 책 조회 3. checkIn()메소드 호출
				// 1. 책이름 입력
				System.out.println("반납할 책 이름은?");
				bTitle  = scanner.next();
				//2. 책 조회
				for(idx=1 ; idx< books.length ; idx++) {
