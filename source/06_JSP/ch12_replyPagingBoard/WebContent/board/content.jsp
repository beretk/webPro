...
<body>
	...
	%> 
		<table>
			<caption><%=bid %>번 글번호 상세보기</caption>
			<tr>
			...
			<tr>
				<td colspan="2">
				<button onclick="location.href='<%=conPath%>/board/updateForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">
					수정
				</button>
				<button onclick="location.href='<%=conPath%>/board/deleteForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">
					삭제
				</button>
				<button onclick="location.href='<%=conPath%>/board/replyForm.jsp?bid=<%=bid%>&pageNum=<%=pageNum%>'">
					답변
				</button>
