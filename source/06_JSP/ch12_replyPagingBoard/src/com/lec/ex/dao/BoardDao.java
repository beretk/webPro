...
public class BoardDao {
	...
	public int writeBoard(BoardDto dto) {
		...
		try {
			...
			System.out.println(e.getMessage() + " - 원글쓰다 예외발생 :" + dto);
		}finally{
			try {
				if(pstmt!=null)pstmt.close();
				if(conn!=null)conn.close();
	...
	//5. bID로 DTO가져오기(글수정 FORM, 답변글쓰기 FORM) - 조회수 올리지 않음
	...
	// 원그에 대한 정보 : bgroup, bstep, bindent
	...
	public int replyBoard(BoardDto dto) {
		...
		try {
			...
			System.out.println(e.getMessage() + " - 답변글쓰다 예외발생 :" + dto);
		}finally{
			try {
				if(pstmt!=null)pstmt.close();
				if(conn!=null)conn.close();
