...
public class BoardDao {
	...
	public ArrayList<BoardDto> listBoard(int startRow, int endRow){
		...
		try {
			...
			while(rs.next()) {
				...
				String bconten = rs.getString("bcontent");
				...
				dtos.add(new BoardDto(bid, bname, btitle, bconten, bdate, bhit, bgroup, bstep, bindent, bip));
			}
	...
	public int writeBoatd(BoardDto dto) {
		int result = FAIL;
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public BoardDto getBoardNotHitUp(int bid) {
		...
		try {
			...
			if(rs.next()) {
				...
				int bindent = rs.getInt("bindent");
				String bip = rs.getString("bip");
				dto = new BoardDto(bid, bname, btitle, bconten, bdate, bhit, bgroup, bstep, bindent, bip);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		} finally {
