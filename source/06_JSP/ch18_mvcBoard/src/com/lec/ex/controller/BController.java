...
public class BController extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 들어온 요청에 따른 일을 하고 뷰단으로 froward
		String uri = request.getRequestURI(); // "/ch18_mvcBoard/list.do"
		String conPath = request.getContextPath(); // "/chq8_mvcBoard"
		...
		if(command.equals("/list.do")) { // 글목록 list.do ? pageNum=2 
		...
		}else if(command.equals("/writeView.do")) { // 글쓰기 form 태그 페이지(뷰단)
			viewPage = "board/write.jsp";
			writeMode = 1;
		}else if(command.equals("/write.do")) { // 글쓰기 DB에 저장
			if(writeMode==1) {
				service = new BWriteService();
				service.execute(request, response); // request에 writeResult추가
				writeMode = 0;
			}
			viewPage = "list.do";
			...
			viewPage = "content.do?after=u"; // 수정후 상세보기 페이지로 넘어갈때 hit수 증가 하지 않음
