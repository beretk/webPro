...
public class Frontcontroller extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Service service = null; // InsertService, SelectService, UpdageService, DeleteService객체가 할당
		if(command.equals("/insert.do")) {
			...
			// update.do로 요청이 들어오면 할 일을 호출
			service = new UpdateService();
			service.execute(request, response);// request에 updateResult추가
			viewPage = "select.do"; // update.do 처리후 select.do로
