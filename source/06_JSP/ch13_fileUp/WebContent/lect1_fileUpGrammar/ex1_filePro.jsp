	...
	<%
		...
		try{
			...
			while(paramNames.hasMoreElements()){
				...
				System.out.println("파라미터이름:" + param + ",서버에 저장된파일명:" + filename
						+ ",사용자가 첨부한 파일이름:" + originalFilename);
			}	
		}catch(IOException e){
		...
		try{
			...
			if(severFile.exists()){
				...
				System.out.println("첨부 파일이 없어서 복사 안함");
			}
		}catch(Exception e){
			System.out.println(e.getMessage());
		}finally{
		...
		if(filename!=null){
	%>	
		<h3>첨부한 오리저널 파일이름: <%=originalFilename %></h3>	
